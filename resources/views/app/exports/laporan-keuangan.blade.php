...
<table class="table table-striped table-bordered table-hover">
    <thead class="thead-dark">
        ...
        <tr>
            @foreach ($kategori as $data)
                <th class="text-center align-middle font-weight-normal">
                    {{ 'Rp' . number_format($data['anggaran'], 0, ',', '.') }}</th>
            @endforeach
            <th class="text-center align-middle font-weight-normal">{{ 'Rp' . number_format($totalAnggaran, 0, ',', '.') }}
            </th>
        </tr>
    </thead>
    <tbody>
        @foreach ($siswa as $index => $row)
            <tr>
                ...
                @endphp
                <td class="text-right align-middle font-weight-bold">
                    {{ $sisa !== 0 ? 'Rp' . number_format($sisa, 0, ',', '.') : '-' }}</td>
        ...
        <tr>
            ...
            @endforeach
            <td>{{ $totalAnggaranSiswa->sum('total_biaya') !== 0 ? 'Rp' . number_format($totalAnggaranSiswa->sum('total_biaya'), 0, ',', '.') : '-' }}
            </td>
            <td colspan="2" class="text-center">
                {{ $totalSisa >= 0 ? 'Rp' . number_format($totalSisa, 0, ',', '.') : '-' }}</td>
        </tr>
    </tbody>
</table>
